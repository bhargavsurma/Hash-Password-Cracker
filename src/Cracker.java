...
public class Cracker {
	...
	public static void main(String[] args) throws FileNotFoundException,NoSuchAlgorithmException, IOException
	{
		...
		try {
			...
			String filename1 = "shadow.txt";
			String filename2 = "common-passwords.txt";

			String[] salt = ck.readLines(filename1);
			String[] compass = ck.readLines(filename2);
			...
			for (k = 0; k < salt1.length; k++)
			{
				for (i = 0; i < compass.length; i++) 
				{
					...
					String MD5_ad1 = MD5Shadow.crypt(compass[i], salt1[k]);
					//System.out.println(MD5_ad1);

					for (j = 0; j <= salt.length-1; j++) 
					{

						if (MD5_ad1.equalsIgnoreCase(salt[j].substring(18,40))) 
						{
